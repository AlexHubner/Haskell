...
typeCheckP :: Program -> R [Enviroment]
typeCheckP (Progr defs) = do
  env <- typeCheckDefs defs
  return env

typeCheckC :: Environment -> ClassDeclaration -> R ClassDeclaration
typeCheckC environemnt (ClassD id members) = do
   let environment' = updateShallow environemnt id (Class id members)
   members' <- mapM (typeCheckM environment') members
   return (ClassD id members')


{--typeCheckC :: Environment -> ClassDeclaration -> R Environment
typeCheckC environment (ClassD id members) = do
   let environment' = updateClass environment id (ClassD id members)
   members' <- mapM (typeCheckM environment') members
   return $ updateClass environment' id (Class id members')--}
...
updateClass :: Environment -> Ident -> ClassDeclaration -> R Environment
updateClass (sc,c) id (ClassD _ _) = let r = updateShallow c id (TClass id) in
                                                     case r of
                                                        OK ctx -> OK (sc,ctx)
                                                        Erro msg -> Erro msg

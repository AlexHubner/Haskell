...
typeCheckClass env (c:cs) = let env' = typeCheckClass' env c in
    case env' of
        (Erro s) -> Erro s
        (OK env) -> typeCheckClass env cs


typeCheckClass' :: Environment -> ClassDeclaration -> R Environment
typeCheckClass' env (ClassD id []) = OK env
typeCheckClass' env (ClassD id (m:ms)) = let env' = typeCheckM env m in
    case env' of
        (Erro s) -> Erro s
        (OK env) -> typeCheckClass' env (ClassD id ms)
...
tinf environment x  =  case x of
    ...
    ENew classN    -> lookupDeepType environment classN
    EMthCall obj mth lexp -> let r = lookupDeepType environment obj in
                                  case r of
                                     OK (TClass classN) -> if (length lexp == 0)
                                                              then let r2 = lookupDeepType environment mth in
                                                                      if (r2 == OK Tvoid)
                                                                         then OK Tvoid
                                                                         else Erro ("Metodo " ++ show mth ++ " nao eh do tipo void")
                                                                else Erro ("Metodo " ++ show mth ++ " nao recebe parametros")

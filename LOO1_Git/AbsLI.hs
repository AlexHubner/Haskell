...
data MethodDeclaration = Mth Type Ident [Decl] [Stm]
  deriving (C.Eq, C.Ord,  C.Read)

data Attribute = Attr Type Ident
  deriving (C.Eq, C.Ord,  C.Read)  

data ClassDeclaration = ClassD Ident [MemberDeclaration]
  deriving (C.Eq, C.Ord, C.Read)

data MemberDeclaration = MthD MethodDeclaration
                       | AttrD Attribute
  deriving (C.Eq, C.Ord,  C.Read) 
